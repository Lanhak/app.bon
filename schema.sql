...
CREATE TABLE IF NOT EXISTS vip_keys (
 ...
 status VARCHAR(10) NOT NULL DEFAULT 'active' CHECK (status IN ('active','disabled','expired')),
 user_id BIGINT REFERENCES users(id) ON DELETE SET NULL,
 note VARCHAR(255),
 created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
 updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE INDEX IF NOT EXISTS idx_key_user ON vip_keys(user_id);
...
CREATE TABLE IF NOT EXISTS wallet_requests (
 id BIGSERIAL PRIMARY KEY,
 user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
 request_type VARCHAR(10) NOT NULL CHECK(request_type IN ('deposit','withdraw')),
